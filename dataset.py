...
class LinearDynamicalDatasetABCD(IterableDataset):
    def __init__(self, nx=5, nu=1, ny=1, seq_len=500, strictly_proper=True, dtype="float32", normalize=True):
        super(LinearDynamicalDataset).__init__()
        self.nx = nx
        self.nu = nu
    ...
    def __iter__(self):
        while True:  # infinite dataset
            ...
            yield torch.tensor(y), torch.tensor(u), torch.tensor(sys.A.astype(self.dtype)), \
                torch.tensor(sys.B.astype(self.dtype)), torch.tensor(sys.C.astype(self.dtype)), \
                torch.tensor(sys.D.astype(self.dtype))
...
class WHDatasetABCD(IterableDataset):
    def __init__(self, nx=5, nu=1, ny=1, seq_len=600, random_order=True,
                 strictly_proper=True, normalize=True, dtype="float32",
                 fixed_system=False, system_seed=None, data_seed=None, **mdlargs):
        super(WHDataset).__init__()
    ...
    def __iter__(self):
        ...
        while True:  # infinite dataset
            ...
            yield torch.tensor(y), torch.tensor(u), G1, G2
